...
// ПИНЫ ДЛЯ ESP8266 (NodeMCU / Wemos D1 Mini)
#define S_LEFT  D3
#define S_UP    D4
#define S_RIGHT D0
#define S_DOWN  D6
#define S_SEL   D7 
#define BEEP    D5 
#define LED_PIN BUILTIN_LED // Обычно D4
...
void setup() {
  ...
  // Инициализация I2C для ESP8266 (SDA=D2, SCL=D1)
  Wire.begin(D2, D1);
  ...
  // Пины кнопок. Важно: D3 и D4 имеют встроенные подтяжки на плате
...
void loop() {
  // Читаем кнопки (на ESP8266 нажатие = LOW)
...
void runApp() {
  ...
  if (inGames) {
    ...
    switch(gIdx) {
     ...
     if (mIdx == 1) { 
        display.print("LED: "); display.print(digitalRead(LED_PIN) ? "OFF" : "ON"); // Инвертировано для BUILTIN_LED
        if(btnUp) digitalWrite(LED_PIN, LOW); if(btnDown) digitalWrite(LED_PIN, HIGH);
     } else {
